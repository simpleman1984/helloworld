...
public class ServiceConfig<T> extends AbstractServiceConfig {
    ...
    private void doExportUrlsFor1Protocol(ProtocolConfig protocolConfig, List<URL> registryURLs) {
        ...
        this.urls.add(url);
        logger.debug("添加服务Url:" + url);
        //如果路由不为空，则创建对应的路由协议 
        if(routers != null && !"".equals(routers)){
        	String address = registries.get(0).getAddress();
        	String routerUrl = routers + "://0.0.0.0/"+interfaceName+"?category=routers";
        	RegistryFactory registryFactory = ExtensionLoader.getExtensionLoader(RegistryFactory.class).getAdaptiveExtension();
    		Registry registry = registryFactory.getRegistry(URL.valueOf(address));
    		
//    		URL url = URL.valueOf("xxx://0.0.0.0/com.DemoService?category=routers&dynamic=false&rule=222222");
    		URL _routerUrl = URL.valueOf(routerUrl);
    		registry.register(_routerUrl);
        	logger.debug("添加路由Url:" + _routerUrl);
        }
    }
